...
import '../../../Utility/ip_utility.dart';
...
class _BookDetailsState extends State<BookDetails>
  ...
  void initState() {
    ...
    final bookId = int.tryParse(parts[0]) ?? 0;
    final initialPage = Storage.instance.getReadingProgress("book", bookId);
                ...
                builder: (context, data, _) {
                  ...
                  return PageView.builder(
                    ...
                    onPageChanged: (val) {
                      ...
                      final parts = widget.input.split(',');
                      final bookId = int.tryParse(parts[0]) ?? 0;
                      Storage.instance.saveReadingProgress("book", bookId, val);
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      "Current: ${(pageController.hasClients ? (pageController.page?.toInt() ?? 0) : 0)}",
                      style: TextStyle(
                        color: getTextColor(),
      ...
      if (bookResponse.status ?? false) {
        ...
        if (flipBookUrl?.isNotEmpty ?? false) {
          ...
          if (details?.is_bought ?? false) {
            Navigation.instance.navigateAndReplace('/flipBook', args: {
              'url': flipBookUrl,
              'title': details?.title ?? 'Flip Book',
            });
          } else {
    ...
    showDialog(
      ...
      builder: (ctx) {
        return WillPopScope(
          ...
          child: AlertDialog(
            title: Text('Permission Denied',style: TextStyle(color: Colors.black,fontSize: 16.sp,fontWeight: FontWeight.bold),),
            content: Text('You do not have permission to read this book.',style: TextStyle(color: Colors.black,fontSize: 14.sp),),
            actions: [
              TextButton(
                child: const Text('OK'),
                onPressed: () {
  ...
  Future<void> fetchData() async {
    try {
      ...
      // Call read-count API with public IP and bookId
      IpUtility.getPublicIp().then((ip) {
        if (ip != null) {
          ApiProvider.instance.postBookReadCount(ip, bookId).then((response) {
            debugPrint('Read count API response: ${response.message}');
          });
        }
      });
